...
public class EventDemo {
	...
	public static void main(String[] args) {
		...
		*/
		...
		//for loop to create, iterate, and print results of three Event objects
		for(int i = 0; i < e.length; i++) {
			e[i] = new Event();
			eventNum = eventNumber();
			e[i].setEventNumber(eventNum);
			guestNum = numberOfGuest();
			e[i].setGuest(guestNum);
			eventType = eventTypePrompt();
			e[i].setEventType(eventType);
		}
		...
		while(userInput > 4) {
			try {
				System.out.println("Must be between 1,2,3,4");
